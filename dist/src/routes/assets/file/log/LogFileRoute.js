...
const LogFileRequest_1 = require("../../../requests/LogFileRequest");
...
let LogFileRoute = class LogFileRoute extends inversify_express_utils_1.BaseHttpController {
    ...
    async getAllLogsForAgent(agentId, res, forwardCallback) {
        try {
            const logFiles = await this.logFileService.getAllLogs(new index_1.AgentId(agentId));
            res.send(logFiles);
            return res;
        }
        catch (err) {
            index_2.logger.error(`Error while getting agent logs: ${err}`);
    ...
    async createLogFile(agentId, logFileRequest, res, forwardCallback) {
        try {
            const logFile = await this.logFileService.createLogFile(new index_1.AgentId(agentId), new Date(logFileRequest.creationTime), new Date(logFileRequest.lastUpdate), logFileRequest.lastCertifiedLine, logFileRequest.exists, logFileRequest.readable, logFileRequest.location, logFileRequest.hashValue);
            ...
            index_2.logger.error(`Error while creating agent : ${err}`);
            res.sendStatus(HttpStatus_1.HttpStatus.INTERNAL_SERVER_ERROR);
            forwardCallback(err);
        }
    }
    async getLogFileAsset(agentId, logFileId, res, forwardCallback) {
        try {
            const logFile = await this.logFileService.getLogFile(new index_1.AgentId(agentId), new LogFileId_1.LogFileId(logFileId));
            if (!logFile) {
                index_2.logger.error(`Cannot find log file asset with id : ${logFileId}`);
